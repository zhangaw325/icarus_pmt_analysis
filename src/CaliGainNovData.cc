...
void CaliGainNovData::fitGainCurve(TFile* ofile){
    ...
    if(ratio > 0.01){
      ...
      fitLowCharge_expGaus();
      plotGainCurveLowCharge_expGaus(ofile,flag_lowcharge);
    }
    else{
...
void CaliGainNovData::SaveJointFitParameters(bool flag){
  ...
  if(flag==true) fout<<ds.ndf-ds.npars_lowcharge<<endl;
  else fout<<ds.ndf-ds.npars_highcharge<<endl;
