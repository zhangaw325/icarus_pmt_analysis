...
void CaliGainNovData::fitGainCurve(TFile* ofile){
    ...
    if(ratio > 0.01){
      ...
      //fitLowCharge_expGaus_improve();
      //plotGainCurveLowCharge_expGaus_improve(ofile,flag_lowcharge);
      fitLowCharge_fullExpr_Single();
      plotGainCurveLowCharge_fullExpr_Single(ofile, flag_lowcharge);
    }
    else{
      ...
      //fitHighCharge_improve();
      //plotGainCurveHighCharge_improve(ofile,flag_lowcharge);
      fitHighCharge();
      plotGainCurveHighCharge(ofile,flag_lowcharge);
    }

    SaveJointFitParameters(flag_lowcharge);
...
// save the joint fit parameters
void CaliGainNovData::SaveJointFitParameters(bool flag){
  string histname ( ds.ghist[0]->GetName() );
  string filename = histname + "_fitPars.txt";
  fstream fout(filename.c_str(), ios::out);
  fout<<histname<<"\t lowcharge_flag\t"<<flag<<"\t";
  fout<<"chi2\t"<<minChi2<<"\tndf\t";
  if(flag==true){
    //fout<<ds.ndf-ds.npars_lowcharge<<endl;
    //fout<<vresult[0]<<"\t"<<verrors[0]<<"\t"
    //    <<vresult[1]<<"\t"<<verrors[1]<<"\t"
    //    <<vresult[2]<<"\t"<<verrors[2]<<"\t"
    //    <<vresult[3]<<"\t"<<verrors[3]<<"\t"
    //    <<vresult[1]+vresult[4]<<"\t"<<TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4])<<"\t"
    //    <<vresult[5]<<"\t"<<verrors[5]<<"\t"
    //    <<vresult[6]<<"\t"<<verrors[6]<<"\t"
    //    <<vresult[1]+vresult[4]+vresult[7]<<"\t"<<TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4]+verrors[7]*verrors[7])<<"\t";
        for(int i=0; i<(int)vresult.size(); i++){
          fout<<vresult[i]<<"\t"<<verrors[i]<<"\t";
        }
  }
  else{

    fout<<ds.ndf-ds.npars_highcharge<<endl;
...
// plot gain curve (high charge)
void CaliGainNovData::plotGainCurveHighCharge_improve(TFile* ofile, bool flagLowCharge){
    ...
    //    gGain->SetPointError(i, 0, verrors[voltages.size()*i+1]/1.6);
    // }
    gGain->SetPoint(0, voltages[0], vresult[1]/1.6);
    gGain->SetPoint(1, voltages[1], (vresult[1]+vresult[1*3+1])/1.6);
    gGain->SetPoint(2, voltages[2], (vresult[1]+vresult[1*3+1]+vresult[2*3+1])/1.6);
    gGain->SetPointError(0, 0, verrors[1]/1.6);
    gGain->SetPointError(1,0,TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4])/1.6);
    gGain->SetPointError(2,0,TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4]+verrors[7]*verrors[7])/1.6);
    gGain->Draw("ALP");
    gGain->SetMarkerStyle(8);
    // fit the gain curve 
    TF1* fGain = new TF1("fGain","[0]*TMath::Power(x,[1])",1,2000);
    ...
    for(int i=0; i<(int)chargehist.size(); i++){
      ...
      //func[i]->SetParNames("mu","q0","sigma","amp.");
      if(i==0)
        func[i]->SetParameters(vresult[0],vresult[i*3+1],vresult[i*3+2],vresult[i*3+3]);
      if(i==1)
        func[i]->SetParameters(vresult[0],vresult[1]+vresult[4],vresult[i*3+2],vresult[i*3+3]);
      if(i==2)
        func[i]->SetParameters(vresult[0],vresult[1]+vresult[4]+vresult[7],vresult[i*3+2],vresult[i*3+3]);
...
// plot gain curve (low charge), full_exp
void CaliGainNovData::plotGainCurveLowCharge(TFile* ofile, bool flagLowCharge){
    ...
    for(int i=0; i< (int)voltages.size(); i++){
        ...
        gGain->SetPointError(i, 0, verrors[i*7+1]/1.6);
    }
    gGain->Draw("ALP");
    gGain->SetMarkerStyle(8);
    // fit the gain curve 
...
// plot gain curve (low charge), full_exp
void CaliGainNovData::plotGainCurveLowCharge_fullExpr_Single(TFile* ofile, bool flagLowCharge){
    ...
    gGain->GetYaxis()->SetTitle("Gain (10^{7})");
    for(int i=0; i< (int)voltages.size(); i++){
        gGain->SetPoint(i, voltages[i], vresult[i*8+1]/1.6);
        gGain->SetPointError(i, 0, verrors[i*8+1]/1.6);
    }
    gGain->Draw("ALP");
    gGain->SetMarkerStyle(8);
    // fit the gain curve 
    ...
    for(int i=0; i<(int)lowchargehist.size(); i++){
      ...
      sprintf(gname,"myresponsefunc%d",i);
      func[i] = new TF1(gname,myChargeFunctionWithRealPedestal_obj,-1, 20, 8);
      //func[i]->SetParameter(0, vresult[0]);
      for(int m=0; m<8; m++){
        func[i]->SetParameter(m, vresult[i*8+m]);
      }
      func[i]->SetNpx(600);
      func[i]->SetLineColor(i+1); func[i]->SetLineStyle(2);
      func[i]->Draw("same");
    ...
    for(int i=0; i<(int)lowchargehist.size();i++){
      sprintf(gname, "%d, %.2f #pm %.2f, %.2f #pm %.2f, %.2f #pm %.2f", i+1,
                 vresult[i*8+0], verrors[i*8+0],
                 vresult[i*8+1], verrors[i*8+1],
                 vresult[i*8+2], verrors[i*8+2]);
      pt->AddText(gname);
    }
    pt->Draw();
    pt->SetBorderSize(1);
    ...
    for(int i=0; i<(int)chargehist.size();i++){
        sprintf(gname, "%.2f #pm %.2f, %.2f #pm %.2f, %.2f #pm %.2f, %.2f #pm %.2f, %.2f #pm %.2f ",
                   vresult[i*8+3], verrors[i*8+3],
                   vresult[i*8+4], verrors[i*8+4],
                   vresult[i*8+5], verrors[i*8+5],
                   vresult[i*8+6], verrors[i*8+6],
                   vresult[i*8+7], verrors[i*8+7]);
        pt1->AddText(gname);
    }
    pt1->Draw();
    pt1->SetBorderSize(1);
...
// plot gain curve (low charge), exp-mod Gaus
void CaliGainNovData::plotGainCurveLowCharge_expGaus(TFile* ofile, bool flagLowCharge){
    ...
    gGain->GetYaxis()->SetTitle("Gain (10^{7})");
    for(int i=0; i< (int)voltages.size(); i++){
        gGain->SetPoint(i, voltages[i], vresult[i*3+1]/1.6);
        gGain->SetPointError(i, 0, verrors[i*3+2]/1.6);
    }
    gGain->Draw("ALP");
    gGain->SetMarkerStyle(8);
    // fit the gain curve 
    ...
    sprintf(gname,"cChargeDis_bd%d_ch%d",m_board,m_ch);
    cCharge->SetName(gname);
    ChargeFunctionWithPedestal_expGaus myChargeFunctionWithRealPedestal_obj; // hard coded  
    TF1* func[3];
    for(int i=0; i<(int)lowchargehist.size(); i++){
      if(i==0) {
        lowchargehist[i]->Draw();
      ...
      sprintf(gname,"myresponsefunc%d",i);
      func[i] = new TF1(gname,myChargeFunctionWithRealPedestal_obj,-1, 50, 8);
      func[i]->SetParameters(vresult[0], vresult[i*3+1], vresult[i*3+2], vresult[i*3+3], vresult[i*4+10], vresult[i*4+11], vresult[i*4+12], vresult[i*4+13]);
      func[i]->SetNpx(600);
      func[i]->SetLineColor(i+1); func[i]->SetLineStyle(2);
      func[i]->Draw("same");
    }
    ...
    sprintf(gname,"npe: %.2f #pm %.2f", vresult[0], verrors[0]);
    pt->AddText(gname);
    pt->AddText("# , q , #sigma, a:");
    for(int i=0; i<(int)lowchargehist.size();i++){
      sprintf(gname, "%d, %.2f #pm %.2f, %.2f #pm %.2f, %.0f #pm %.0f", i+1,
                 vresult[i*3+1], verrors[i*3+1],
                 vresult[i*3+2], verrors[i*3+2],
                 vresult[i*3+3], verrors[i*3+3]);
      pt->AddText(gname);
    }
    pt->Draw();
    pt->SetBorderSize(1);
    ...
    pt1->AddText("other parameters");
    for(int i=0; i<(int)chargehist.size();i++){
        sprintf(gname, "%.2f #pm %.2f, %.2f #pm %.2f, %.2f #pm %.2f, %.0f #pm %.0f",
                   vresult[i*4+10], verrors[i*4+10],
                   vresult[i*4+11], verrors[i*4+11],
                   vresult[i*4+12], verrors[i*4+12],
                   vresult[i*4+13], verrors[i*4+13]);
        pt1->AddText(gname);
    }
    pt1->Draw();
    pt1->SetBorderSize(1);
...
void CaliGainNovData::fitLowCharge_fullExpr_Single(){
  //ds.npars_lowcharge = 8*3;
  initializeParVectors(24);//(ds.npars_lowcharge);
  myChargeFunctionWithRealPedestal myfunc_obj;
  TF1* hfit[3];
  char name[50];
  for(int i=0; i<3; i++) {
    sprintf(name, "myfit%d",i);
    hfit[i] = new TF1(name, myfunc_obj,-1,50,8);
    hfit[i]->SetParNames("npe","q1","sigma1","q0","sigma0","c0","w","a");
    hfit[i]->SetParameters( 0.5,
                            ds.ghist[i]->GetMean()/1.6,
                            0.4,
                            0.0,
                            0.1,
                            1.0,
                            0.3,
                            ds.ghist[i]->Integral()*TMath::Exp(-1.0*ds.ghist[i]->GetMean()/1.6)
                          );
    ds.ghist[i]->Fit(name,"RQ","",-1,20);
    cout<<"fit "<<ds.ghist[i]->GetName()<<"\tchi2/ndf "
        << hfit[i]->GetChisquare()<<"\t"
        << hfit[i]->GetNDF()<<endl;
    for(int j=0; j<8; j++){
      vresult[8*i+j] = hfit[i]->GetParameter(j);
      verrors[8*i+j] = hfit[i]->GetParError(j);
      cout<<vresult[8*i+j]<<"\t"<<verrors[8*i+j]<<"\t";
    }
    cout<<endl;

    hfit[i]->SetLineColor(i+1); hfit[i]->SetLineStyle(2);
  }
}
...
void CaliGainNovData::fitLowCharge_expGaus(){ // pedestal with expoentially-modified Guassian
    ...
    double vstart[n_pars]={20, 
                       0.5, 0.5, lowchargehist[0]->Integral()/400.0*40,
                       0.5, 0.5, lowchargehist[1]->Integral()/400.0*40,
                       0.5, 0.5, lowchargehist[2]->Integral()/400.0*40,
...
void CaliGainNovData::fitHighCharge(){
    ...
    double minVal[n_pars]={0, 0.001, 0.001, 0, 0.001, 0.001, 0, 0.001, 0.001, 0};
...
// fit high charge distributions
void CaliGainNovData::fitHighCharge_improve(){
    ...
    double vstart[n_pars]={20, 
                       1.0, 0.5, chargehist[0]->Integral()/2.0,
                       0.25, 0.5, chargehist[1]->Integral()/2.0,
                       0.25, 0.5, chargehist[2]->Integral()/2.0} ;
    double step[n_pars]={0.1, 0.01, 0.1, 10, 0.01, 0.1, 10, 0.01, 0.1, 10};
    double minVal[n_pars]={0, 0.01, 0.001, 0, 0.0, 0.001, 0, 0.0, 0.001, 0};
    double maxVal[n_pars]={50, 10, 10,chargehist[0]->Integral()*5.0, 
                          10, 10,chargehist[1]->Integral()*5.0, 
                          10, 10,chargehist[2]->Integral()*5.0};
    string parName[n_pars]={"npe",
                            "q1", "w1", "a1",
                            "q2", "w2", "a2",
                            "q3", "w3", "a3"
    ...
    //for(int i=0; i<(int)chargehist.size(); i++){
      vstart[0] = chargehist[1]->GetMean()/1.6;
    //}
    ...
    TMinuit* minimizer = new TMinuit(n_pars);
    // the fit function is set
    minimizer->SetFCN(CaliGainNovData::myfcn_improve);
    //  
    for(int i=0; i<n_pars; i++){
      minimizer->DefineParameter(i, parName[i].c_str(), vstart[i], step[i], 
                             minVal[i], maxVal[i]);
...
// this is the real function that is used at this moment
// for computing the chi2, using the ideal response of a PMT
double CaliGainNovData::idealResponse_improve(double npe,
                                      double Q1, double w1, double a1,            
                                      double Q2, double w2, double a2,
                                      double Q3, double w3, double a3)
{
  ...
  // I have these parameters:
  //double npe = mu; // mean npe
  double q[npoints]; q[0]=Q1; q[1]=q[0]+Q2; q[2]=q[1]+Q3;
  double sigma[npoints]; sigma[0]=w1; sigma[1]=w2; sigma[2]=w3;
  double amp[npoints]; amp[0]=a1; amp[1]=a2; amp[2]=a3;
...
// this is the function to be called by the fitter
// inside it will call another function to do real calculations
void CaliGainNovData::myfcn_improve(int &npar, double * deriv, double &f, double * par, int iflag){
  f = fitobj->idealResponse_improve(par[0], par[1], par[2], par[3], par[4], par[5], par[6], par[7], par[8], par[9]);
}
...
void CaliGainNovData::myfcnWithPedestal_expGaus_improve(int &npar, double * deriv, double &f, double * par, int iflag){
  f = fitobj->ResponseWithPedestal_expGaus_improve(par[0], par[1], par[2], par[3], par[4], par[5], par[6], par[7], par[8], par[9], par[10], par[11], par[12], par[13], par[14], par[15], par[16], par[17], par[18], par[19], par[20], par[21]);
}


// fit high charge distributions
void CaliGainNovData::fitLowCharge_expGaus_improve(){ // pedestal with expoentially-modified Guassian

   // in this fit I try to require q1 to be increasing as HV increase
    ...
    const int n_pars = 22;//ds.npars_lowcharge;//10;//1 + 3*n ;
    double vstart[n_pars]={20, 
                       0.1, 0.5, lowchargehist[0]->Integral()/400.0*40,
                       0.05, 0.5, lowchargehist[1]->Integral()/400.0*40,
                       0.05, 0.5, lowchargehist[2]->Integral()/400.0*40,
                       0.0, 0.1, lowchargehist[0]->Integral(bin1[0],bin2[0])/10.0*4, 0.1,
                       0.0, 0.1, lowchargehist[1]->Integral(bin1[1],bin2[1])/10.0*4, 0.1,
                       0.0, 0.1, lowchargehist[2]->Integral(bin1[2],bin2[2])/10.0*4, 0.1};
    double step[n_pars]={0.02,
    ...
    TMinuit* minimizer = new TMinuit(n_pars);
    // the fit function is set
    minimizer->SetFCN(CaliGainNovData::myfcnWithPedestal_expGaus_improve);
    //  
    for(int i=0; i<n_pars; i++){
      minimizer->DefineParameter(i, parName[i].c_str(), vstart[i], step[i], 
                             minVal[i], maxVal[i]);
...
// this is the function that is used at this moment
// for computing the chi2, using the response with pedestal, expoentially modified guassian
double CaliGainNovData::ResponseWithPedestal_expGaus_improve(double npe,
                                      double Q1, double w1, double a1,            
                                      double Q2, double w2, double a2,
                                      double Q3, double w3, double a3,
                                      double pedm1, double pedw1, double pedamp1, double pedtau1,
...
// plot gain curve (low charge), exp-mod Gaus
void CaliGainNovData::plotGainCurveLowCharge_expGaus_improve(TFile* ofile, bool flagLowCharge){
   ...
   // }
    ...
    for(int i=0; i<(int)lowchargehist.size(); i++){
      ...
      if(i==0)
        func[i]->SetParameters(vresult[0], vresult[i*3+1], vresult[i*3+2], vresult[i*3+3], vresult[i*4+10], vresult[i*4+11], vresult[i*4+12], vresult[i*4+13]);
      if(i==1)
        func[i]->SetParameters(vresult[0], vresult[1]+vresult[i*3+1], vresult[i*3+2], vresult[i*3+3], vresult[i*4+10], vresult[i*4+11], vresult[i*4+12], vresult[i*4+13]);
      if(i==2)
        func[i]->SetParameters(vresult[0], vresult[1]+vresult[4]+vresult[i*3+1], vresult[i*3+2], vresult[i*3+3], vresult[i*4+10], vresult[i*4+11], vresult[i*4+12], vresult[i*4+13]);
      func[i]->SetNpx(1200);
      func[i]->SetLineColor(i+1); func[i]->SetLineStyle(2);
      func[i]->Draw("same");
    }
    ...
    pt->AddText("# , q , #sigma, a:");
    for(int i=0; i<(int)lowchargehist.size();i++){
      if(i==0) { sprintf(gname, "%d, %.2f #pm %.2f, %.2f #pm %.2f, %.0f #pm %.0f", i+1,
                 vresult[i*3+1], verrors[i*3+1],
                 vresult[i*3+2], verrors[i*3+2],
                 vresult[i*3+3], verrors[i*3+3]);
      }
      if(i==1) { sprintf(gname, "%d, %.2f #pm %.2f, %.2f #pm %.2f, %.0f #pm %.0f", i+1,
                 vresult[1]+vresult[4], TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4]),
                 vresult[i*3+2], verrors[i*3+2],
                 vresult[i*3+3], verrors[i*3+3]);
      }
      if(i==2) { sprintf(gname, "%d, %.2f #pm %.2f, %.2f #pm %.2f, %.0f #pm %.0f", i+1,
                 vresult[1]+vresult[4]+vresult[7], TMath::Sqrt(verrors[1]*verrors[1]+verrors[4]*verrors[4]+verrors[7]*verrors[7]),
                 vresult[i*3+2], verrors[i*3+2],
                 vresult[i*3+3], verrors[i*3+3]);
      }
