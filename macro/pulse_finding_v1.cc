...
void pulse_finding_v1(string metadatafile="metadata1.txt"){ //_LedOff
  ...
  */
  ...
  while(fin_meta>>run){ // process one run
    ...
    for(int e=0; e<tchain->GetEntries(); e++)
    {
      ...
      for(int board=0; board<nboards; board++)
      {
        for(int channel=0; channel<nchannels; channel++)
        {
          ...
          waveform->CountingPulses(of_wave);
    ...
    char ofilename[100]; sprintf(ofilename, "Result_run%d_pulseFinding.root", run);
